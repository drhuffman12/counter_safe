...
Spectator.describe CounterSafe::Shared do
  ...
  describe "#initialize" do
    it "does not raise" do
      expect { CounterSafe::Shared.new }.not_to raise_error
    end
  end

  describe "#value" do
    let(counter) { CounterSafe::Shared.new }
  ...
  describe "when we have multiple counters" do
    let(counter1) { CounterSafe::Shared.new }
    let(counter2) { CounterSafe::Shared.new }
    let(key) { Faker::Name.name }
    let(qty_times) { rand(10) }
